...
export default {
  ...
  data: function () {
    return {
      ...
      declineReason: null,
      declineOptions: [
        { text: 'Something is wrong with the file', value: 'Something is wrong with the file.' },
        { text: 'Unusable quality', value: 'Unusable quality.' },
        { text: 'Number of stems is to large', value: 'Number of stems is to large.' },
        { text: 'Lacking details', value: 'Lacking details.' },
        { text: 'Unreasonable request', value: 'Unreasonable request.' },
  ...
  methods: {
    ...
    onContinue: async function () {
      ...
      try {
        this.$emit('status-changed', 'rejected', { reject_reason: declineReason })
        this.dialog = false
      } catch (err) {
        this.throwError(err)
      }
