<template>
  <v-card :no-body="competition.status === 'closed'">
    ...
    <v-card-text class="text-center">
      ...
      <h3 v-if="competition.status !== 'closed'">DAYS LEFT UNTIL NEXT ROUND OF ELIMINATION!</h3>
      <v-progress-circular
        indeterminate
        color="primary"
        v-if="loading"
      ></v-progress-circular>
      <div v-else>
        <p v-if="competition.status !== 'closed'" class="text-uppercase text-muted ls-1 h5">
        ...
        <v-btn dark id="start_button" class="m-0" color="success" @click="competitionStep(1)" v-if="competition.registration_status && !form.song_promotion_link">
          {{ buttonText }}
        </v-btn>

        <div v-if="competition.level" class="mt-2 text-center">
          <template v-if="form.song_promotion_link">
          <span class="mt-2">
        ...
        <div v-if="competition.status !== 'closed'" class="row pb-2">
          <div class="col-lg-12 text-center">
            <div v-if="competitionCloseTime">
              <div v-if="canUserEnterCompetition">
                ...
                <p v-if="form.id" class="mb-0">Refer 3 friends and enter to win an extra $250</p>
              </div>
...
export default {
  ...
  computed: {
    ...
    isRegistrationStarted () {
      const startDate = momentTimezone.tz(this.competition.registrationStartDate, 'America/Los_Angeles')
  ...
  methods: {
    ...
    async getTrackDetails () {
      this.loading = true
      try {
        let songRes = (await Vue.$http.get(`v1/competition/${this.competition.slug}/song`))
