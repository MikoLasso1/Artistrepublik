<template>
  <div id="filters" v-click-outside="onClickOutside">
    ...
    <div v-bind:class="{opened: isOpened}" id="filter-wrap">
      <div class="form-group">
        <label for="name">Outlet Name</label>
        <input type="text" class="form-control" name="name" v-model="filters.name" placeholder="Outlet name" />
      </div>
      <div class="row">
        <div class="col text-center">
          <v-btn dark color="primary" @click="toggleOpen">Filter</v-btn>
        </div>
      </div>
    </div>
  </div>
