  ...
  <div class="w-100">
    ...
    <v-row>
      <v-col md="3" cols="6" class="payout-button">
        <v-btn color="success" :disabled="!enablePayout" @click="openPayoutModal">
          Payout
        </v-btn>
      </v-col>
    </v-row>
    <payout-modal ref="payoutModal" :settings="settings" :payout-amount="payout_amount" :last-payout="last_payout" @payout="updatePayout" />
  </div>
</template>
...
import PayoutModal from '@/components/EliteReviews/OrderSummary/PayoutModal'
...
export default {
  ...
  computed: {
    enablePayout: function () {
      return this.payout_amount > 0 && this.reviewer
    }
  },
  methods: {
    fetchStats: async function () {
      ...
      } = await Vue.$http.get(config.REVIEW_REPUBLIK_API_URL + '/api/v1/reviewers/' + this.reviewer + '/stats')
